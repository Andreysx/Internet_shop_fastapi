...
from app.schemas import Order as OrderSchema, OrderList as OrderListSchema, OrderCheckoutResponse
...
@router.post(path="/checkout", response_model=OrderCheckoutResponse, status_code=status.HTTP_201_CREATED)
async def checkout_order(db: AsyncSession = Depends(get_async_db), current_user: UserModel = Depends(get_current_user)):
    ...
    """
    cart_result = await db.scalars(select(CartItemModel).options(selectinload(CartItemModel.product))
                                   .where(CartItemModel.user_id == current_user.id).order_by(CartItemModel.id))
    ...
    for cart_item in cart_items:
        ...
        order_item = OrderItemModel(product_id=cart_item.product_id,
...
@router.get(path="/", response_model=OrderListSchema)
async def list_orders(
        page: int = Query(1, ge=1), page_size: int = Query(10, ge=1, le=100), db: AsyncSession = Depends(get_async_db),
        current_user: UserModel = Depends(get_current_user)):
    """
    Возвращает заказы текущего пользователя с простой пагинацией(offset-based).
    """
...
@router.get(path="/{order_id}", response_model=OrderSchema)
