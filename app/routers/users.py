...
from app.schemas import UserCreate, UserRoleUpdate, User as UserSchema, RefreshTokenRequest
from app.db_depends import get_async_db
from app.auth import hash_password, verify_password, create_access_token, create_refresh_token, is_admin
...
async def access_token(
    ...
    """
    Обновляет access-токен, принимая старый refresh-токен в теле запроса и возвращает новый  access-токен.
    """
...
async def refresh_token(
    ...
    Обновляет refresh-токен, принимая старый refresh-токен в теле запроса и возвращает новый refresh-токен.
    """
    credentials_exception = HTTPException(
        status_code=status.HTTP_401_UNAUTHORIZED,
        detail="Could not validate refresh token",
